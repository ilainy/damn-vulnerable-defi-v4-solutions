...
合约基于标准 **ERC4626 金库协议** 开发，额外实现了 ERC3156 闪电贷功能。
...
1. 遵循 ERC4626 规范，通过份额（share）与资产（asset）的汇率，实现用户存取款；
...
```
ERC20 代币原生支持 transfer 转账，攻击者可以不经过合约任何业务函数（deposit/闪电贷），直接向金库合约地址转账代币。  
  
这会造成致命的数据错位：
  
1. 正规用户存款：调用 deposit，合约同时增加代币余额 + 增发用户份额，两者数据同步；  
