...
- **Aave 闪电贷**：借入海量 WETH+stETH，调用add_liquidity铸造巨量 Curve LP；后续调用remove_liquidity触发时序漏洞，最大化拉高瞬时virtual_price，确保虚假价格击穿用户 300% 超额抵押
...
4. **池子操作无回调风险**：忽略 remove_liquidity 执行间隙的外部调用机会，虽然清算函数具备重入锁，但重入锁无法抵御跨合约发起的只读重入。
...
1. **增加池子状态一致性校验**：清算读取 virtual_price 之前，快照 Curve 池子 totalSupply 与各币种余额；读取价格后再次比对。若前后快照不一致，代表合约处于执行中途的中间状态，直接拒绝清算。

2. **使用 TWAP 时间加权价格替代瞬时 get_virtual_price**：不再依赖单时点瞬时视图价格，采用多区块加权均价，单次交易触发的瞬时价格扰动很难显著影响 TWAP，大幅降低时序操纵空间。

3. **增加价格波动阈值风控**：设置单区块价格最大波动上限，检测到价格短时间剧烈跳变时临时暂停清算功能，作为兜底防护。

4. **头寸安全二次校验**：超额抵押头寸清算前，增加二次风控校验，避免逻辑漏洞导致的恶意清算。
